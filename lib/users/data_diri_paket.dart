...
class DataDiriPaket extends StatefulWidget {
  final String data;
  final String paket;

  const DataDiriPaket({Key key, @required this.data, @required this.paket})
      : super(key: key);
...
class _DataDiriPageState extends State<DataDiriPaket> {
  var nama;
  var alamat;
  var tglLahir;
  var jK;
  ...
  Future<void> uploadFileKK(String filePath) async {
    ...
    try {
      await firebase_storage.FirebaseStorage.instance
          .ref('uploads/${_imageKK.path.split('/').last}')
          .putFile(file);
  ...
  Future<void> uploadFileAkte(String filePath) async {
    ...
    try {
      await firebase_storage.FirebaseStorage.instance
          .ref('uploads/${_imageAkte.path.split('/').last}')
          .putFile(file);
  ...
  CollectionReference dataDiri =
      FirebaseFirestore.instance.collection('DataJamaah');
  ...
  Future<void> updateUserData() async {
    ...
    return dataDiri
        ...
        .add({
          ...
          'nama_paket': preferences.getString('paket'),
        })
        .then((value) => {
        ...
        .catchError((error) =>
            {Navigator.pop(context), print("Failed to add user: $error")});
  }

  CollectionReference pemb =
      FirebaseFirestore.instance.collection('Pembayaran');

  Future<void> sendPembayaran() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    return pemb
        .add({
          'nama': controllerNama.text.toString(),
          'alamat': controllerAlamat.text.toString(),
          'paket': preferences.getString('paket'),
          'id_user': prefUid
        })
        .then((value) => {addTagihan()})
        .catchError((error) =>
            {Navigator.pop(context), print("Failed to add : $error")});
  }

  CollectionReference tagihan =
      FirebaseFirestore.instance.collection('TagihanPem');

  Future<void> addTagihan() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    return tagihan
        .doc(prefUid)
        .collection('tagihan')
        .add({
          'kekurangan': preferences.getString('price'),
          'nama_paket': preferences.getString('paket'),
        })
        .then((value) => {addInfoUmroh()})
        .catchError((onError) =>
            {Navigator.pop(context), print("Failed to add : $onError")});
  }

  CollectionReference info = FirebaseFirestore.instance.collection('InfoUmroh');

  Future<void> addInfoUmroh() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    return info
        .doc(prefUid)
        .collection('info')
        .add({
          'nama_paket': preferences.getString('paket'),
          'tanggal': preferences.getString('date'),
          'price': preferences.getString('price'),
          'info': preferences.getString('information')
        })
        .then((value) => {
              Navigator.pop(context),
              Navigator.of(context).pushNamed("/Home"),
              Toast.show("add data success", context,
                  duration: Toast.LENGTH_LONG, gravity: Toast.BOTTOM),
            })
        .catchError((onError) =>
            {Navigator.pop(context), print("Failed to add : $onError")});
  ...
  _getUser() async {
    return FirebaseFirestore.instance
      ...
      if (documentSnapshot.exists) {
        ...
        tglLahir = documentSnapshot.data()['tanggal_lahir'];
        jK = documentSnapshot.data()['jenis_kelamin'];
        noHp = documentSnapshot.data()['no_hp'];

        setState(() {
          controllerNama.text = nama.toString();
          controllerAlamat.text = alamat.toString();
          controllerTanggalLahir.text = tglLahir.toString();
          _dropDownValue = jK;
          controllerNoHp.text = noHp.toString();
        });
      } else {
